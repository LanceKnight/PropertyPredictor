...
target_col = [4]#[x for x in range(8,12)]
...
class MyNet(torch.nn.Module):
	...
	def forward(self, x, edge_index, edge_attr, smiles, batch):
		...
		return Sigmoid()(out), overall_molecule_fp
...
def train(data_loader, debug_mode, target_col, add_noise):
	...
	for data in data_loader:
		...
		if(add_noise):
			...
			out_noise, _ = model(x_noise.float(), data.edge_index, data.edge_attr, data.smiles, data.batch)# use our own x and edge_attr instead of data.x and data.edge_attr
		...
		if(debug_mode):
			mol_fp_np = mol_fp.cpu().detach().numpy()
			mol_fp_df = mol_fp_df.append(pd.DataFrame(mol_fp_np))
			mol_fp_df.to_csv('embeddings.csv')
...
for col in target_col:
	...
	for epoch in tqdm(range(num_epoches)):
		...
		if epoch > add_noise_train_perc * num_epoches:
			add_noise = True
		train(train_loader, epoch==(num_epoches-1), col, add_noise)#epoch==(num_epoches-1))

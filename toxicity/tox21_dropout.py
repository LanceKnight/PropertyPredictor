# added a dropout to the shallow network
import torch
...
def train(data_loader, debug_mode, target_col, rampup_val):
	model.train()
	for data in data_loader:
		#print(f"smi:{data.smiles}")
		x, edge_attr = batch2attributes(data.smiles, molecular_attributes= True)
		...
		out1 = model(data.x.float(), data.edge_index, data.edge_attr, data.smiles, data.batch)# use our own x and edge_attr instead of data.x and data.edge_attr
		...
		unsupervised_loss = torch.nn.MSELoss(out1, out2)
...
for col in target_col:
	...
	for epoch in tqdm(range(num_epoches)):
		...
		train(train_loader, False, col, rampup_val)#epoch==(num_epoches-1))
